...
```{r echo=FALSE, eval = TRUE, fig.align='center', out.width='45%'}
...
trapezoid rule under-estimates.
...
- Let $\Phi(x|\mu,\sigma^2)$ denote the CDF of the Normal distribution with mean $\mu$ and variance $\sigma^2$, and $\varphi(x|\mu,\sigma^2)$ its density.
...
- Location of peak must be located first
  - Often found using Newton or Quasi-Newton methods
- Assumes a symmetric distribution
- Much faster than MCMC
- Assumes unimodal

::: notes
If your data is substantially skewed or multimodal laplace can be very biased
:::
...
```{r echo=FALSE, eval = TRUE, fig.align='center', out.width='50%'}
knitr::include_graphics("rejection.png")
```

::: notes
More nano-banana... I couldn't help myself
:::
