...
- Cloud computing (Keven Haynes) moved to March 26
  - Should be getting emails from Keven, please respond
- We will finish up Bayesian computing after break
...
## The Bayesisan philosophy : a simple motivating example
...
steps = round(rnorm(n, mu_true, sd = sqrt(sigma_y)))
...
::: notes
Frequentist inference would say we are 95% confident that the true number of steps taken by Rollins students lies between 9722 and 10,276 steps.
:::
...
densities_df = tibble(
  ...
  likelihood = dnorm(steps, mu_true, sd = sqrt(sigma_y)),
...
::: notes
Hyperparameters are parameters of the prior distribution.

They determine how the parameters themselves are distributed.
:::
...
$$p(\sigma_y^2|y,X,\beta) \propto p(y|\beta, \sigma^2_y, X)p(\sigma^2_y|A,B)$$
...
We're going to do this using data from a cannabis consumption and driving study in Colorado.
