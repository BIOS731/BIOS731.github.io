...
$$q^*(\beta) = N(m, S)$$
$$S = (\frac{1}{\sigma^2_y}x^Tx + \frac{1}{\sigma^2_b}I)^{-1}$$

$$m = S(\frac{1}{\sigma^2_y}x^Ty)$$
...
\fontsize{10pt}{11pt}\selectfont
Update for $\sigma^2_y$  is

$$q^*(\sigma^2_y) = IG(A_{post}, B_{post})$$
$$A_{post} = A_0 + n/2$$
$$B_{post} = B_0 + 1/2E_{q(\beta)}[(y - x\beta)^2] = B_0 + 1/2\left((y-xm)^2 + tr(x^tx S)\right)$$

--- 

## BLR CAVI Algorithnm:

1. Initialize $\sigma^2_y$
2. Repeat until ELBO converges:
- Update q(\beta) by computing S and m using current $1/\sigma^2_y$
- Update q(\sigma^2) by computing $A_{post}$ and $B_{post}$
