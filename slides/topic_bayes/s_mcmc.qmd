...
ggplot(df, aes(x = x, y = f)) +
  ...
  labs(title = expression(p(x) == frac(e^{-x^2}*(2 + sin(5*x)), Z)),
...
results1 %>%
  ...
  mutate(accepted = as.numeric(accepted),
         prob = zoo::rollmean(accepted, k = 100, fill = NA),
...
```{r, cache = TRUE, echo = FALSE, eval = TRUE, fig.align='center', fig.height = 5, fig.width = 10, message = FALSE}
set.seed(23423)
...
ergodicity means that the chain can move anywhere at each
