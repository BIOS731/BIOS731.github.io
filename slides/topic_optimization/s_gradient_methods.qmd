...
# Load ggplot2 for a professional look
library(ggplot2)
library(tidyr)

# 1. Define the iterations
iterations <- 0:10

# 2. Simulate the Error decay for each type
# Starting error is 0.5 (50cm away from the wall)
linear_error      <- 0.5^(iterations + 1)
superlinear_error <- 0.5^(iterations^1.5) # Grows faster than linear
quadratic_error   <- 0.5^(2^iterations)    # Error is squared each time

# 3. Create a Data Frame
df <- data.frame(
  Iteration = iterations,
  Linear = linear_error,
  Superlinear = superlinear_error,
  Quadratic = quadratic_error
)

# Reshape for plotting
df_long <- pivot_longer(df, cols = -Iteration, names_to = "Type", values_to = "Error")

# 4. Generate the Plot
ggplot(df_long, aes(x = Iteration, y = Error, color = Type, group = Type)) +
  geom_line(linewidth = 1.2) +
  geom_point(size = 3) +
  scale_y_log10(limits = c(1e-15, 1)) + # Log scale is key for convergence!
  theme_minimal() +
  labs(
    title = "Comparison of Convergence Rates",
    x = "Iteration Step",
    y = "Error (Log Scale)",
    color = "Convergence Type"
  ) +
  scale_color_manual(values = c("Linear" = "#56B4E9", "Superlinear" = "#E69F00", "Quadratic" = "#D55E00")) +
...
::: notes
Sometimes $g(\mu_i) = \eta_i$ is called the linear predictor
...
- **observed information**: A function of the sample size $n$. Negative of the Hessian.     
...
**Answer**: No, because $E(g(y|x))\ne g(E(y|x)) = x^T\beta$, since $g$ is nonlinear. This means we cannot transform $y$ by $g$ and then run linear regression.
...
- McCullagh and Nelder (1983) showed that IRLS is equivalent to Fisher scoring in **standard GLMs**.
...
If your algorithm doesn't converge your inference won't have asymptotic properties that holdD
