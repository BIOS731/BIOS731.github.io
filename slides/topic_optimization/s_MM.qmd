...
1. $g(\theta | \theta^t) \le f(\theta) \forall \theta$
2. $g(\theta^t|\theta^t) = f(\theta^t)$

Ascent property: 

$$f(\theta^t) \ge g(\theta^{t+1}) \ge g(\theta^t|\theta^t) = f(\theta^t)$$


::: notes
the ascent property follows directly from the definition of minorization, together with the way the MM update is constructed.
- trick is in defining a majorizing or minorizing function
::: 
...
- **Property of convex functions**: A function $f: R^p \to R$ is $\textbf{convex}$ if for all $x_1, x_2 \in R^p$ and all $\alpha \in [0,1]$,
 $$
...
knitr::include_graphics("quadratic_bound.jpg")
```

::: notes
Gemini/nano banana generated this for me and I think its outrageous
:::
