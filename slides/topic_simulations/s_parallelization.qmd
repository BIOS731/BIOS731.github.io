...
::: notes
Randomly shuffle Y nperm times. Refit regression, record beta at each one.
:::
...
::: notes
Keep features (X values) fixed. Shuffle labels (Y). Retrain model, measure accuracy. Repeat 1000 times.
:::
...
$$\hat{p}\sim Binomial(nperm, p)/nperm$$
and

$$SE(\hat{p}) = \sqrt{\frac{p(1-p)}{nperm}}$$
