...
Ignoring non-negativity, conditions 1--4 give $2n + 2m$ equations in $2n + 2m$ unknowns:
...
\fontsize{10pt}{11pt}\selectfont
**Newton’s method** finds roots of nonlinear systems by repeating:

> *Linearize $F$ around the current point. Solve for the direction to move. Take a step.*

At iteration $k$, the Newton step finds direction $(\delta x, \delta y, \delta w, \delta z)$ by solving:

$$\underbrace{F’(x^k, y^k, w^k, z^k)}_{\text{Jacobian: how fast } F \text{ is changing}} \begin{bmatrix} \delta x \\ \delta y \\ \delta w \\ \delta z \end{bmatrix} = \underbrace{-F(x^k, y^k, w^k, z^k)}_{\text{how far we are from } F=0}$$

- If $F$ is large $\Rightarrow$ we’re far from optimal $\Rightarrow$ take a big step
- If $F \approx 0$ $\Rightarrow$ we’re near optimal $\Rightarrow$ tiny correction needed

::: notes
The Jacobian F’ has a block structure — differentiating each row of F with respect to (x, y, w, z):
F’ = [[A, 0, I, 0], [0, A^T, 0, -I], [Z^k, 0, 0, X^k], [0, W^k, Y^k, 0]]
Rows 1–2 (primal/dual feasibility) are linear in all variables, so their Jacobian blocks are constant (A, I). Rows 3–4 (complementary slackness products) are nonlinear — their Jacobian blocks depend on the current iterate values (X^k, Z^k etc). This is exactly the matrix that appears in the "improved algorithm" slide. If students ask about the Jacobian explicitly, this is what it is — but the key message on the slide is just the structure of the Newton step: solve a linear system at each iteration to find the search direction.
:::

---

## Primal-dual interior point: basic algorithm

\fontsize{10pt}{11pt}\selectfont

Putting it together — the **basic** interior point algorithm:

1. Start from a strictly feasible point $(x^0, y^0, w^0, z^0)$ with all variables $> 0$
2. Solve the Newton system $F'(\cdot)\,\delta = -F(\cdot)$ for the search direction $\delta$
3. Update: $(x,y,w,z) \mathrel{+}= \alpha \cdot \delta$, choosing $\alpha$ small enough that all variables stay $> 0$
4. Repeat until $F \approx 0$

**The catch**: how small does $\alpha$ need to be?

Near the optimum, complementary slackness requires $x_j \cdot z_j \to 0$ — meaning both $x_j$ and $z_j$ approach zero simultaneously. To prevent either from going negative, $\alpha$ must be tiny. The algorithm crawls to a halt near the finish line.

::: notes
This is a clean statement of the basic algorithm before the centering modification. The problem with step size α is real and important — it's not just a mathematical technicality. Near the boundary, the maximum feasible step shrinks to zero, so the algorithm makes essentially no progress on the complementary slackness conditions. This motivates the centering trick on the next slide.
:::

---

## The problem: crawling near the boundary

\fontsize{10pt}{11pt}\selectfont

**1D picture**: suppose we want to solve $x \cdot z = 0$ with $x > 0$ and $z > 0$.

At the optimum, one of them must be 0. But during the algorithm both must stay $> 0$.

```{r, echo = FALSE, eval = TRUE, fig.height = 2.8, fig.width = 7, fig.align = "center"}
library(ggplot2)
x <- seq(0.05, 3, length.out = 200)
df <- data.frame(
  x = rep(x, 3),
  z = c(3/x, 1/x, 0.2/x),
  mu = rep(c("xz = 3 (far from optimal)",
             "xz = 1 (getting closer)",
             "xz = 0.2 (near optimal, tiny steps)"), each = 200)
)
pts <- data.frame(x = c(1.7, 1.0, 0.45), z = c(3/1.7, 1/1.0, 0.2/0.45),
                  mu = c("xz = 3 (far from optimal)",
                         "xz = 1 (getting closer)",
                         "xz = 0.2 (near optimal, tiny steps)"))
ggplot(df, aes(x, z, color = mu)) +
  geom_line(linewidth = 1) +
  geom_point(data = pts, size = 3) +
  coord_cartesian(xlim = c(0, 3), ylim = c(0, 5)) +
  labs(x = "x", y = "z", color = NULL,
       title = "Feasible curves xz = μ; dots = current iterate") +
  theme_minimal(base_size = 9) +
  theme(legend.position = "bottom",
        legend.text = element_text(size = 7))
```

Near the axes ($xz \approx 0$), steps toward the origin must be tiny to keep $x, z > 0$.

::: notes
The curves xz = μ are the "central path" — the set of points where complementary slackness holds at level μ. As μ → 0, the path approaches the axes (the optimal solution). The closer you are to the axes, the smaller the feasible steps. This is the fundamental limitation of the basic algorithm — it requires smaller and smaller step sizes as it converges, making the last part of convergence very slow.
:::
...
**Key idea**: instead of targeting $x_j z_j = 0$, target $x_j z_j = \mu > 0$ and shrink $\mu$ each step.

Replace the CS conditions in $F$ with a relaxed version:
$$XZe = \mu_x e \qquad WYe = \mu_y e$$

At each iteration, set $\mu$ = the **average current complementarity**:
$$\mu_x^k = \frac{(x^k)^T z^k}{n}, \qquad \mu_y^k = \frac{(w^k)^T y^k}{m}$$

When $x_j z_j$ is large, $\mu$ is large — we're far from optimal, take a big step.
When $x_j z_j \approx 0$ for all $j$, $\mu \approx 0$ — we're near optimal, we're done.

**Why this works**: the algorithm now follows the central path (the curve $xz = \mu$) toward the origin, staying away from the axes at each step $\Rightarrow$ large steps allowed throughout.

::: notes
The centering parameter μ has a direct interpretation: it is the duality gap per variable. When μ is large, the primal and dual objectives are far apart. When μ → 0, we're at the optimum. By targeting xz = μ instead of xz = 0, we keep the iterates on the central path and avoid the boundary. This is the key insight of the primal-dual interior point method, and it's what allows convergence in 20–50 iterations on problems of any size. Modern solvers (CPLEX, Gurobi, MOSEK) all use variants of this approach.
:::

---

## The complete interior point algorithm

\fontsize{9pt}{10pt}\selectfont

With centering, the Newton step at iteration $k$ solves:
...
$$\begin{bmatrix}
    ...
    0 & W^k & Y^k & 0
\end{bmatrix}
\begin{bmatrix} \delta x \\ \delta y \\ \delta w \\ \delta z \end{bmatrix}
=
\begin{bmatrix}
    0 \\ 0 \\
    \red{-X^k Z^k e + \mu_x^k e} \\
    \red{-W^k Y^k e + \mu_y^k e}
\end{bmatrix}$$

\red{Only the CS rows change}: replace the target $0$ with $\mu^k e$. Everything else is identical to the basic algorithm.

**Algorithm**:

1. Start from strictly feasible $(x^0, y^0, w^0, z^0)$
2. Compute $\mu^k$ = average complementarity
3. Solve Newton system above for $(\delta x, \delta y, \delta w, \delta z)$
4. Update $(x,y,w,z) \mathrel{+}= \alpha^k \cdot \delta$ (choose $\alpha^k$ to keep all variables $> 0$)
5. Repeat until $\mu^k < \varepsilon$

**Convergence**: ~20–50 iterations regardless of problem size.

::: notes
The complete algorithm is just the basic Newton iteration with μ substituted for 0 in the RHS. The Jacobian is unchanged. The only modification is the target on the right-hand side. The top two rows of the RHS are 0 because we maintain primal and dual feasibility throughout — the algorithm only needs to improve complementary slackness. The stopping criterion is μ < ε (small duality gap), which by strong duality means we're at the optimum. This is the algorithm inside CPLEX, Gurobi, MOSEK, and HiGHS — they use sophisticated variants (Mehrotra's predictor-corrector, etc.) but the core idea is exactly this.
:::
...
\fontsize{10pt}{11pt}\selectfont
Use the LP from class: $\max\ x_1 + x_2$ s.t. $x_1 + 2x_2 \le 100,\ 2x_1 + x_2 \le 100,\ x_1, x_2 \ge 0$
...
::: notes
The carpenter problem: a carpenter makes tables and chairs, limited by lumber and labor hours. Set up the primal (maximize profit), identify the corner points of the feasible region by solving pairwise constraint intersections, and evaluate both the primal and dual objectives at each corner. Students should observe that the dual objective ≥ primal objective at every feasible point (weak duality). At the optimal corner, they should be equal (strong duality). This is a good concrete numerical verification before abstract theory, and also reinforces how to find corner points of a feasible polytope.
:::
