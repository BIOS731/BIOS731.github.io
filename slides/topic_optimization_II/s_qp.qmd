...
**Why is this nonlinear?** Conditions 1--2 are linear, but conditions 3--4 involve products ($x_j z_j$, $w_i y_i$). This is a \textbf{nonlinear} square system $\Rightarrow$ use \textbf{Newton's method}.
...
\fontsize{9pt}{10pt}\selectfont
At iteration $k$, solve $F’(\cdot)\,\delta = -F(\cdot)$ for the search direction $(\delta x, \delta y, \delta w, \delta z)$.

The Jacobian $F’$ (differentiate each block of $F$ w.r.t. each variable group):

$$F’ = \frac{\partial F}{\partial (x,y,w,z)} =
\begin{bmatrix}
    A & 0 & I & 0 \\
    0 & A^T & 0 & -I \\
    Z^k & 0 & 0 & X^k \\
...
- Rows 1--2: linear in all variables $\Rightarrow$ constant Jacobian blocks ($A$, $I$)
- Rows 3--4: nonlinear products $\Rightarrow$ Jacobian blocks depend on current iterate ($X^k$, $Z^k$, ...)

So the Newton step solves:

$$\begin{bmatrix} A & 0 & I & 0 \\ 0 & A^T & 0 & -I \\ Z^k & 0 & 0 & X^k \\ 0 & W^k & Y^k & 0 \end{bmatrix} \begin{bmatrix} \delta x \\ \delta y \\ \delta w \\ \delta z \end{bmatrix} = -F(x^k, y^k, w^k, z^k)$$
...
**Problem**: near the optimum, some $x_j$ or $z_j \to 0$. The step size $\alpha$ must be tiny to keep all variables positive $\Rightarrow$ slow convergence.

**1D intuition**: want to solve $xz = 0$ with $x, z > 0$. Newton step takes us to the boundary ($x=0$ or $z=0$), so $\alpha \to 0$. Instead, aim for $xz = \mu > 0$ and shrink $\mu$ each iteration:

$$\mu = 1 \;\longrightarrow\; \mu = 0.1 \;\longrightarrow\; \mu = 0.01 \;\longrightarrow\; \cdots \;\longrightarrow\; 0$$

Each target keeps us away from the boundary $\Rightarrow$ larger steps allowed.

**Fix**: replace complementary slackness conditions with a relaxed target $\mu > 0$:
...
As $\mu_x, \mu_y\to 0$, this converges to the original optimality conditions. Set:
$$\mu_x^{k+1}=\frac{(x^k)^Tz^k}{n}, \quad \mu_y^{k+1}=\frac{(w^k)^Ty^k}{m}$$
(average complementarity at iteration $k$ — small when near optimal)
