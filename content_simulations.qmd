...
### Slide Deck: Benchmarking and parallelization

[![](./slides/topic_simulations/preview_parallel.png){width=70%}](./slides/topic_reproducible_computing/s_parallelization.pdf)

**Speeding up your code in R**  
[Download slides (PDF)](./slides/topic_simulations/s_parallelization.pdf)
...
### Lab: Benchmarking and parallelization

#### Vectorization

1. Create a function __square_for(x)__ that takes as an argument vector __x__ (of any length), and uses the **for loop** to return vector __y__ such that $$y_i = x_i^2$$ 
2. Create a function __square_sapply(x)__ that does the same thing using **sapply** function. 
3. Create a function __square_vec(x)__ that does the same thing using **\^** operator applied directly to vector __x__. 
4. Create a function __square_vec2(x)__ that does the same thing using * operator applied directly to vector __x__. 
5. Create a vector **x** of $p=100,000$ normal random variables
